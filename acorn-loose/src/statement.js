...
lp.parseTopLevel = function() {
  ...
  node.body = []
  while (this.tok.type !== tt.eof) node.body.push(this.parseStatement())// add statement node to file body 
...
lp.parseStatement = function() {
  ...
  case tt._for:
    ...
    if (isLet || this.tok.type === tt._var || this.tok.type === tt._const) {
      ...
      if (init.acorn_declarations.length === 1 && (this.tok.type === tt._in || this.isContextual("of"))) {
  ...
  case tt._try:
    this.next()
    node.block = this.parseBlock()//ICI { Open scope / block try 
    node.handler = null
    if (this.tok.type === tt._catch) {
      ...
      clause.body = this.parseBlock()//ICI { Open scope / block catch 
      node.handler = this.finishNode(clause, "CatchClause")
    }
    node.finalizer = this.eat(tt._finally) ? this.parseBlock() : null//ICI { Open scope / block finally 
  ...
  case tt.braceL:
    return this.parseBlock()//ICI { Open scope / block 
...
lp.parseBlock = function() {
  ...
  node.body = []
  while (!this.closes(tt.braceR, blockIndent, line, true))
    node.body.push(this.parseStatement())// add statement node to body of block //function == declaration( identifier fct ) + blockstatemtn
  this.popCx()
  this.eat(tt.braceR)
...
lp.parseFunction = function(node, isStatement, isAsync) {
  ...
  node.body = this.parseBlock()//ICI { Open scope / block function 
