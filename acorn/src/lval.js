...
import {_ju_parseBindingAtom, ju_parseBindingAtom, ju_checkLValSimple, ju_checkLVal} from "./julien/extendsAcorn.js"
...
pp.toAssignable = function(node, isBinding, refDestructuringErrors) {
  if (this.options.ecmaVersion >= 6 && node) {
    switch (node.type) {
    case "Identifier":
      if (this.inAsync && node.name === getLabelFromTokenType(ju_tt.await))
    ...
    case "Property":
      ...
      if (node.kind !== getLabelFromTokenType(ju_tt.init)) this.raise(node.key.start, "Object pattern can't contain getter or setter")
...
pp.parseBindingAtom = function() {
  let fct = (_obj) => {
    let res_node = null
    if (_obj.options.ecmaVersion >= 6) {
      switch (_obj.type) {
      case tt.bracketL:
        let node = _obj.startNode()
        _obj.next()
        node.elements = _obj.parseBindingList(tt.bracketR, true, true)
        res_node = _obj.finishNode(node, "ArrayPattern")
        break

      case tt.braceL:
        res_node = _obj.parseObj(true)
        break
      }
      _ju_parseBindingAtom(_obj, _obj.type, res_node)
    }
    if (!res_node) res_node = _obj.parseIdent()
    return res_node
  }
  return ju_parseBindingAtom(this, fct) // JU ADD
}
...
pp.checkLValSimple = function(expr, bindingType = BIND_NONE, checkClashes) {
  ...
  ju_checkLValSimple(this, expr, isBind, bindingType)// JU ADD
  ...
  case "Identifier":
    ...
    if (isBind) {
      if (bindingType === BIND_LEXICAL && expr.name === getLabelFromTokenType(ju_tt.let))
...
pp.checkLValPattern = function(expr, bindingType = BIND_NONE, checkClashes) {
  ...
  ju_checkLVal(this, expr.type, fct)// JU ADD
}
